...
/*        This function will find all the loop cut points in a FSMD            */
...
findpaths(FSMD *fsmd,PATHS_LIST *paths, var_list* V) {
	...
	//isTempCutPoint is the final cutpointS of FSMD
	...
	printf("lastState = %d\n", lastState);
	// now making only outer loops and branching states as cutpoint
	for (i = 0; i < fsmd->numstates; i++){
...
void //DISPLAY_A_PATH <- for finding fast
FindExplicitLeak(PATH_ST *p, int idx,  var_list* V1) {
	printf("\nentry\n");
	int i, j, k;
//	***************************************initializing gamma ****************************************************************************************
	/*for (i = 0; i < 20; i++){
...
//	***************************************Reading salpha and ralpha****************************************************************************************	
	...
	while(temp1!=NULL) {
			...
			if (store[i] != -1){
				...
				for (j = 0; j < V1->no_of_elements; j++){
					if (store[i] == V1->var_val[j]){
						//printf("haha = %d %d\n", j, lhsIndex);
						variableUpdate = 1;
						info[idx].dependency[lhsIndex][j] = 1;
					}
				}
			}
		}
		...
		if (variableUpdate == 0){
			//printf("haha = %d\n", lhsIndex);
			info[idx].dependency[lhsIndex][0] = 1;
		}
...
void detectLoops(PATHS_LIST *pl, FSMD* M1){
	int i, j;
	int loopNumber = 0;
	for(i = 0; i < M1->numstates; i++){
		if (M1->states[i].isLoop == 1 && M1->states[i].isTempCutPoint == 1){
			...
			int k = 0;
			//gammaLoop[loopNumber].pathIndices[0] = i;
			gammaLoop[loopNumber].startState = i;
				...
				if (pl->paths[j].start == i){
	      		...
	      		gammaLoop[loopNumber].numberOfPaths = k;
	      		loopNumber ++;
...
void findLoopGamma1(int i){
	...
	printf("number of levels traversed = %d\n", numberOfLevels + 1);
	printf("Gamma\n");
	for (l = 0; l < sz && gammaLoop[i].numberOfPaths > 0; l++){
		for (m = 0; m < sz; m++){
			printf("%d ",gammaLoop[i].n[l][m]);
		}
		printf("\n");
...
void traceTrace(PATHS_LIST *pl, int end, int final, int traceNumber, int traceGamma[20][20], int traceDependency[20][20], int pathIndices[20], int x, FSMD* M1, var_list* V, int t){
	...
	*/
	...
	for (l = 0; l < 20; l++){
		for (m = 0; m < 20; m++){
			traceGamma[l][m] = gammaLoop[idx].n[l][m];
			traceDependency[l][m] = gammaLoop[idx].dependency[l][m];
		}
	}
	/*printf("hi \n");
	for (l = 0; l < 15; l++){
...
void PreAnalysis(FSMD* M1, var_list* V, PATHS_LIST *pl){
	...
	for (i = 0; i < M1->numstates; i++){
		if (M1->states[i].isTempCutPoint == 1 && M1->states[i].isLoop == 0){
			cutpoints[j] = i;
			j++;
		}
	}
...
int leakage[1000][1000];
int vis[100];
int c = 0;
int explicit[100][100], implicit[100][100];
void graphTraversal(FSMD* M1, PATHS_LIST *pl, int final, int end, var_list* V1, int explicitleakage[sz][sz], int implicitleakage[sz][sz], int netDependency[sz][sz], int netN[sz][sz]){
	...
	c ++;
	if (c == 10000){
		/*int totalLeakage = 0, i;
	    	for (i = 0; i < sz; i++){
	    		int j;
	    		int isLeaky = 0;
	    		int leak = 0;
	    		for (j = 0; j < sz; j++){
	    			printf("%d ", leakage[i][j]);
	    			leak += leakage[i][j];
	...
	if (final == end){
		...
		for (i = 0; i < sz; i++){
			for (j = 0; j < sz; j++){
				if (explicitleakage[i][j] == 1){
					explicit[i][j] = 1;
					leakage[i][j] = 1;
				}
				if (implicitleakage[i][j] == 1){
					implicit[i][j] = 1;
					leakage[i][j] = 1;
				}
			}
		...
		if (M1->states[pl->paths[i].start].isLoop == 1){
			...
			for(j = 0; j < pl->numpaths; j++){
				...
				if (pl->paths[j].start == end){
					printf("Entered here\n");
					FindExplicitLeak( &pl->paths[j], j, V1);
					idx1 = j;
				}
			}
		//printf("outward = %s", M1->states[outward].state_id);

			int idx;
			for (j = 0; j < 50; j++){
				if (gammaLoop[j].startState == end){
					idx = j;
					break;
				}
			}
			...
			printf("TOTAL PATHS = %d\n", gammaLoop[idx].numberOfPaths);
			
			
			
		    	/*
		    	fetch_expression(pl->paths[1].condition, store, 0);
			printf("\n");
			int l, m;
			for (l = 0; l < 20; l++){
				for (m = 0; m < 20; m++){
					printf("%d ", pl->paths[1].Gamma.n[l][m]);
					
				}
				printf("\n");
			}
			fetch_expression(gammaLoop[idx].paths[0].condition, store, 0);
			printf("\n");
			for (l = 0; l < 20; l++){
				for (m = 0; m < 20; m++){
					printf("%d ", gammaLoop[idx].paths[0].Gamma.n[l][m]);
			...
			for (l = 0; l < sz; l++){
				for (m = 0; m < sz; m++){
					...
					gammaLoop[idx].n[l][m] = 0;
					gammaLoop[idx].dependency[l][m] = 0;
				}
				//printf("\n");
			}
			for (j = 0; j < gammaLoop[idx].numberOfPaths; j++){
				//printf("hello = %d %d\n", gammaLoop[i].paths[j].start, gammaLoop[i].paths[j].end);
				for (l = 0; l < sz; l++){
					for (m = 0; m < sz; m++){
						if (info[gammaLoop[idx].pathIndices[j]].n[l][m] == 1){
							gammaLoop[idx].n[l][m] = info[gammaLoop[idx].pathIndices[j]].n[l][m];
				//			printf("hehehehehe\n");
						}
						if (info[gammaLoop[idx].pathIndices[j]].dependency[l][m] == 1){
							gammaLoop[idx].dependency[l][m] = info[gammaLoop[idx].pathIndices[j]].dependency[l][m];
						}
					}
				}
			}
			...
			int currDependency[sz][sz], currN[sz][sz];
			for (k = 0; k < sz; k++){
				for (j = 0; j < sz; j++){
					currDependency[k][j] = 0;
					currN[k][j] = 0;
				}
			} 
			...
			for (k = 0; k < sz; k++){
				for (j = 0; j < sz; j++){
					currDependency[k][j] = gammaLoop[idx].dependency[k][j];
					currN[k][j] = gammaLoop[idx].n[k][j];
				}
			}
			
			// propagation from previous paths
			for (k = 0; k < sz; k++){
				int takeFromCurr = 0;
				for (j = 0; j < sz; j++){
					if (currDependency[k][j] == 1){
						takeFromCurr = 1;
						break;
					}
				}
				if (takeFromCurr){
					...
					for (j = 0; j < sz; j++){
						...
						if (currDependency[k][j] == 1){
							...
							for (m = 0; m < sz; m++){
								if (explicitleakage[j][m] == 1){
									leaky = 1;
									printf("i AM HERE\n");
									gammaLoop[idx].dependency[k][m] = 1;
									gammaLoop[idx].n[k][m] = 1;
								}
							}							
						}
					}
					...
					/*for (j = 0; j < sz; j++){
						...
						if (currDependency[k][j] == 1){
							for (m = 0; m < sz; m++){
								if (implicitleakage[j][m] == 1){
									implicitleakage[k][m] = 1;
									j = sz;
									break;
								}
							}
...
// propagation across parallel paths of loop
			findLoopGamma1(idx);
			...
			for (k = 0; k < sz; k++){
				...
				if (takeFromCurr){
					printf("here\n");
					for (j = 0; j < sz; j++){
						netDependency[k][j] = gammaLoop[idx].dependency[k][j];
						explicitleakage[k][j] = gammaLoop[idx].n[k][j];
					}
				}
			}
			...
			for (j = 0; j < gammaLoop[idx].numberOfPaths; j++){
				...
				printf("j = %d, num of paths = %d", j, gammaLoop[idx].numberOfPaths);
				printf("*************************************************************************************************************\n");
			    	fetch_expression(pl->paths[gammaLoop[idx].pathIndices[j]].condition, store, 0);
			    	printf("\n");
			    	printf("*************************************************************************************************************\n");
					...
					if (store[p] != -1){
						...
						for (q = 0; q < V1->no_of_elements; q++){
							...
							if (store[p] == V1->var_val[q] && netDependency[q][index] == 1){
								isHighInCondition = 1;
							}
			...
			if (z != 0){
				...
				for (j = 0; j < V1->no_of_elements; j++){
					...
					// Take first path
					r_alpha *temp1 = pl->paths[gammaLoop[idx].pathIndices[1]].transformations;
					...
					// Other paths to compare
					for (m = 1; m < gammaLoop[idx].numberOfPaths; m++){
						r_alpha *temp3 = NULL;
						r_alpha *temp2 = pl->paths[gammaLoop[idx].pathIndices[m]].transformations;
						while (temp2 != NULL){
			...
			printf("expilicit leakage\n");
			for (l = 0; l < sz; l++){
				for (m = 0; m < sz; m++){
					printf("%d ", explicitleakage[l][m]);
					
				}
				printf("\n");
			}
		printf("implicit leakage\n");
			for (l = 0; l < sz; l++){
				for (m = 0; m < sz; m++){
					printf("%d ", implicitleakage[l][m]);
			...
			for (j = 0; j < sz; j++){
				for (k = 0; k < sz; k++){
					printf("%d ", gammaLoop[idx].dependency[j][k]);
				}
				printf("\n");
			}*/
			...
			for (j = 0; j < 20 && outward[j] != -1; j++){
				...
				graphTraversal(M1, pl, final, outward[j], V1, explicitleakage, implicitleakage, netDependency, netN);
			}
			i = pl->numpaths;
			
		}
		else{ 
			//if the path is not inside a loop
			...
			FindExplicitLeak(&pl->paths[i], i, V1);
			printf("\n");
			int currDependency[sz][sz], currN[sz][sz];
			...
			for (k = 0; k < sz; k++){
				for (j = 0; j < sz; j++){
					currDependency[k][j] = info[i].dependency[k][j];
					currN[k][j] = info[i].n[k][j];
				}
			}
     ...
     // find implicit leakage


			// Fetching the condition to store TC set (Tainted Condition)
			int store[1000];				
			for (j = 0; j < 1000; j++){
				store[j] = -1;
			}
		    ...
		    //	printf("hehehe2\n");
				...
				if (store[p] != -1){
					...
					for (q = 0; q < V1->no_of_elements; q++){
					//	printf("p = %d, q = %d\n", p , q);
						if (store[p] == V1->var_val[q] && (temp[q][index] == 1)){
							printf("caught high1 = %s\n", V1->var_string[q]);
							isHighInCondition = 1;
						}
			...
			// culprit branch check, candidate variables are stored in leaky array
			// checkLeaky verifies existence of any Candidate Variable and isHighInCondition verifies TC (tainted condition) is null or not
			int checkLeaky = 0;
			for (p = 0; p < 100; p++){
			...
			printf("For high Variable = %s leakage is mentioned below\n", V1->var_string[highVariableIndex[0]]);
	...
	int l, m;
		...
		printf("implicit leakage\n");
			...
			/*for (j = 0; j < sz; j++){
				int propagateImplicit = 0;
				for (k = 0; k < sz; k++){
					if (implicitleakage[j][k] == 1 && pl->paths[i].Gamma.dependency[k][j]3){
						//explicitleakage[j][k] = 1;
						break;
					}
				}
				if (propagateImplicit){
					for (k = 0; k < sz; k++){
						explicitleakage[j][k] = 1;
					}
				}
			...
			for (j = 0; j < sz; j++){
				for (k = 0; k < sz; k++){
					printf("%d ", implicitleakage[j][k]);
				}
				printf("\n");
			}
			printf("explicit leakage\n");
			for (j = 0; j < sz; j++){
				for (k = 0; k < sz; k++){
					printf("%d ", explicitleakage[j][k]);
				}
				printf("\n");
			}*/
			/*
			...
			// do oring
			//printf("hi99999000000 = %s\n", M1->states[pl->paths[i].end].state_id);
			graphTraversal(M1, pl, final, pl->paths[i].end, V1, explicitleakage, implicitleakage, netDependency, netN);
			//i = pl->numpaths;
...
// driver function of the algo
void
Associate_R_and_r_alpha(FSMD* M1, PATHS_LIST *pl, var_list* V1, int t)
{
		clock_t tt;
    tt = clock();
   	int i;
	...
	for(i = 0; i < pl->numpaths; i++){
  	...
  	for(i = 0; i < pl->numpaths; i++){
   		printf("path start = %s, path end = %s\n", M1->states[pl->paths[i].start].state_id, M1->states[pl->paths[i].end].state_id);
      		...
      		for (x = 0; x < sz; x++){
			for (y = 0; y < sz; y++){
				pl->paths[i].Gamma.n[x][y] = 0;
			}
		}
		for (x = 0; x < sz; x++){
			pl->paths[i].Gamma.c[x] = 0;
		}*/
    	}
   	...
   	for (i = 0; i < pl->numpaths; i++){
		if (pl->paths[i].start == 0){
			int implicitleakage[sz][sz], explicitleakage[sz][sz], netDependency[sz][sz], netN[sz][sz];
			int j, k;
			for (j = 0; j < sz; j++){
				for (k = 0; k < sz; k++){
					implicitleakage[j][k] = 0;
					explicitleakage[j][k] = 0;
					netDependency[j][k] = 0;
					netN[j][k] = 0;
				}
			}
			graphTraversal(M1, pl, lastState, 0, V1, implicitleakage, explicitleakage, netDependency, netN);
		}
	}
...
printf("count is = %d\n", totalLeakage);
	...
	int h = 0;
	printf("Total Leaky High input count \n");
	while( !feof(f1) )
	{
		char var[sz];
		fscanf( f1, "%s", var);
	...
	totalLeakage=0;
	for (i = 0; i < h; i++){
		printf("For high Variable = %s leakage is mentioned below\n", V1->var_string[highVariableIndex[i]]);
		int j;
		for (j = 0; j < sz; j++){
			if (leakage[j][highVariableIndex[i]] == 1){
				printf("%s\n", V1->var_string[j]);
				totalLeakage++;
			}
		}
	}
	printf("total unique leaky variables = %d\n\n", totalLeakage);
