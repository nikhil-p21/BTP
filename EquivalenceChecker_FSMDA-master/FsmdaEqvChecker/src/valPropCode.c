...
printf("REached here111\n");
    ...
    printf("trace information = %d\n", trace_info[1].totalTraces);
    for (i = 0; i < trace_info[0].totalTraces; i++){
    	int l, m;
    	printf("First Program\n");
    	for (l = 0; l < 18; l++){
		for (m = 0; m < 18; m++){
			printf("%d ", trace_info[0].trace[i][l][m]);	
		}
		printf("\n");
	}
	printf("\n\nSecond Program\n");
	for (l = 0; l < 18; l++){
		for (m = 0; m < 18; m++){
			printf("%d ", trace_info[1].trace[i][l][m]);	
		}
		printf("\n");
	}
	printf("\n\n");
